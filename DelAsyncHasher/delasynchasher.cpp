...
class DelAsyncRunnable : public QObject, public QRunnable
{
    Q_OBJECT

public:
    DelAsyncRunnable(QIODevice *device, QCryptographicHash::Algorithm algorithm)
...
public:
    ...
    DelAsyncRunnable *m_runnable = nullptr;
};
    ...
    if (d->m_source != source) {
        ...
        if (source.isLocalFile()) {
            ...
            if (file->open(QIODevice::ReadOnly)) {
                emit started();
                if (d->m_asynchronous) {
                    d->m_runnable = new DelAsyncRunnable(file, d->m_algorithm);
                    connect(d->m_runnable, &DelAsyncRunnable::finished, this, &DelAsyncHasher::setHashValue, Qt::QueuedConnection);
                    connect(d->m_runnable, &DelAsyncRunnable::progress, this, &DelAsyncHasher::hashProgress, Qt::QueuedConnection);
            ...
            connect(d->m_reply, &QNetworkReply::finished, this, [d, this]{
                if (d->m_reply->error() == QNetworkReply::NoError) {
                    if (d->m_asynchronous) {
                        d->m_runnable = new DelAsyncRunnable(d->m_reply, d->m_algorithm);
                        connect(d->m_runnable, &DelAsyncRunnable::finished, this, &DelAsyncHasher::setHashValue, Qt::QueuedConnection);
                        connect(d->m_runnable, &DelAsyncRunnable::progress, this, &DelAsyncHasher::hashProgress, Qt::QueuedConnection);
                        QThreadPool::globalInstance()->start(d->m_runnable);
                    } else {
...
void DelAsyncHasher::setSourceText(const QString &sourceText)
{
    ...
    if (d->m_sourceText != sourceText) {
        ...
        if (d->m_asynchronous) {
            ...
            d->m_runnable = new DelAsyncRunnable(buffer, d->m_algorithm);
            connect(d->m_runnable, &DelAsyncRunnable::finished, this, &DelAsyncHasher::setHashValue, Qt::QueuedConnection);
            connect(d->m_runnable, &DelAsyncRunnable::progress, this, &DelAsyncHasher::hashProgress, Qt::QueuedConnection);
            QThreadPool::globalInstance()->start(d->m_runnable);
        } else {
