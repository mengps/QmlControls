...
QList<QString> DelColorGenerator::generate(DelColorGenerator::Preset color, bool light, const QColor &background)
{
    return generate(presetToColor(color), light, background);
}

QList<QString> DelColorGenerator::generate(const QColor &color, bool light, const QColor &background)
{
    ...
    if (!light) {
        ...
        std::for_each(g_darkColorMap.begin(), g_darkColorMap.end(), [&patterns, &darkColorString, background](const std::tuple<int, qreal> &value){
            ...
            darkColorString.append(mix(background.isValid() ? background : QColor(0x141414), patterns[index], amount).name());
        });
